...
#define IP_AND_UDP_LEN 28
#define QUIC_Q043_OFFSET 9
#define QUIC_Q043_MINSIZE QUIC_Q043_OFFSET + 5
...
#endif
{
  ...
  unsigned char tmp_udp_data[5] = {0, 0, 0, 0, 0};
  ...
  if (VERBOSE >= LBM_INFO) {
    if (inner_ip->protocol == IPPROTO_UDP)
      pr_info("Received marked UDP pkt\n");
    else
  ...
  if (icmp_header) {
    ...
    for (int i = 0; i < quic_ports_count; i++) {
      if (udp_header->dest == quic_ports[i]) {
        found = 1;
        break;
      }
    }
    ...
    if (found == 0) {
      if (VERBOSE >= LBM_NOTICE)
        pr_info("Non QUIC UDP packet (port %d)\n", ntohs(udp_header->dest));
      goto accept;
    }

    if (VERBOSE >= LBM_DEBUG)
      pr_info("data %p data_len %d len %d head %p tail %u end %u\n", pskb->data,
              pskb->data_len, pskb->len, pskb->head, pskb->tail, pskb->end);

    if ((udp_header->len >= htons(QUIC_Q043_MINSIZE)) &&
        (pskb->len > (QUIC_Q043_MINSIZE + IP_AND_UDP_LEN))) {
      udp_data =
          skb_header_pointer(pskb,
                             ((unsigned char *)udp_header - pskb->data) +
                                 sizeof(struct udphdr) + QUIC_Q043_OFFSET,
                             4, tmp_udp_data);

      if (udp_data && (VERBOSE >= LBM_NOTICE)) {
        pr_info("UDP data: %hhx %hhx %hhx %hhx \n\n", udp_data[0], udp_data[1],
                udp_data[2], udp_data[3]);
      }

      if ((udp_data) && ((udp_data[0] == 'Q') && (udp_data[1] == '0') &&
                         (udp_data[2] == '4') && (udp_data[3] == '3'))) {
        if (VERBOSE >= LBM_NOTICE)
          pr_info("UDP QUIC init packet found, accepting\n");
        goto accept;
      }
    }
  } else if (tcp_header == NULL || (tcp_header->syn && !tcp_header->ack)) {
  ...
  // For UDP it means that target server ignored QUIC init packet (IP_CT_NEW)
  if (ct == NULL || ctinfo == IP_CT_NEW) {
    if (VERBOSE >= LBM_NOTICE) {
      pr_info("reflect %s ct valid:%d ctinfo:%d\n",
              (udp_header == NULL) ? "TCP" : "UDP", ct == NULL ? 0 : 1,
              (int)ctinfo);
    }
    goto reflect;
  }

  // tcp.port is union with udp.port, so displayed data will be correct for
  // UDP/QUIC in the same way as for TCP
  if (VERBOSE >= LBM_DEBUG) {
  ...
  if (udp_header != NULL) {
    // we have some kind of valid UDP conntrack info, accepting package

    if (VERBOSE >= LBM_NOTICE) {
      pr_info("UDP accept ct valid:%d ctinfo:%d\n", ct == NULL ? 0 : 1,
              (int)ctinfo);
    }
    ...
    if (VERBOSE >= LBM_DEBUG) {
      ...
      if (tcp_state < sizeof(tcp_conntrack_names) &&
        ...
        pr_info("Pkt State { TCP: %s, IP: %s } ASSURED_BIT %d\n",
                tcp_conntrack_names[tcp_state], ip_conntrack_names[(int)ctinfo],
...
#endif
{
  ...
  outer_ip->check = 0;
  outer_ip->check = ip_fast_csum((unsigned char *)outer_ip, outer_ip->ihl);
